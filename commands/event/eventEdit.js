...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((option) =>
      option
        .setName("몇일뒤")
        .setDescription("몇일 뒤에 시작할지 수정합니다.")
        .setRequired(true)
    )
    .addNumberOption((option) =>
      option
        .setName("몇시")
        .setDescription("24시 기준 (0~23)")
        .setRequired(true)
    )
    .addNumberOption((option) =>
  ...
  async execute(interaction) {
    ...
    const eventName = interaction.options.getString("내전이름") ?? "";
    ...
    try {
      ...
      if (recentEvent && !recentEvent.isDone) {
        ...
        if (updatedEvent) {
          const eventEmbed = createEventEmbed(updatedEvent);
